...
export function ChainSelector({
  ...
  const options = useMemo(
    () =>
      chainConfigs.map((chainConfig) => fromChainConfigToOption(chainConfig)),
    [chainConfigs]
  );
  const value = useMemo(
    () =>
      selectedChainConfig
        ? fromChainConfigToOption(selectedChainConfig)
        : undefined,
    [selectedChainConfig]
  );
  ...
  const handleOnChange = useCallback(
    (option: Option | null) => {
      const newSelectedAccount = option
        ? chainConfigs.find((chainConfig) => chainConfig.name === option.value)
        : undefined;
